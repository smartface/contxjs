 ...
 */
export default function makeStylable({
  component,
  classNames = "",
  defaultClassNames = "",
  userStyle = {},
...
class Stylable extends Actor {
  constructor(component, name, classNames, defaultClassNames, userStyle) {
    ...
    this.defaultClassNames = defaultClassNames
      ? _findClassNames(defaultClassNames)
      : [];
  ...
  computeAndAssignStyle(style, force = false) {
    ...
    var isScrollView =
      _component.layout &&
      _component instanceof require("@smartface/native/ui/scrollview");
  ...
  applyStyles(force = false) {
    this.computeAndAssignStyle(merge(
      {},
      this.waitedStyle,
      this.userStyle
    ), force);
  ...
  resetClassNames(classNames = []) {
    this.classNames = [];
    [...this.initialClassNames, ...classNames].forEach(
      this.addClassName.bind(this)
    );
    this.isDirty = true;
    return this;
  }
