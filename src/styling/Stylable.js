...
const toStringUtil = require("../util/toStringUtil");

const _findClassNames = classNames =>
  findClassNames(classNames).reduce(
    (acc, item) => (!item && []) || acc.concat([item.join("")]),
    []
  );
...
function componentAssign(component, key, value) {
  if (value !== null && value instanceof Object && componentObjectProps[key]) {
    Object.keys(value).forEach(k =>
      componentAssign(component[key], k, value[k])
    );
  } else {
    component[key] = value;
  }
 ...
 */
function makeStylable({ component, classNames = "", userStyle = {}, name }) {
...
class Stylable extends Actor {
  constructor(component, name, classNames, userStyle) {
    ...
    this.classNames = this.initialClassNames.slice();
    this.styles = {};
    this.inlinestyles = {};
    this.isDirty = true;
  ...
  computeAndAssignStyle(style, force = false) {
    ...
    var _component = this.getName().endsWith("_statusBar")
    ...
    if (this.safeArea) {
      const getNotEmpty = (v, y) =>
        v !== undefined ? v : (y !== undefined && y) || null;

      const addValstoSafeAreaIfExists = (val, willAdd) =>
        typeof willAdd === "number" && typeof val === "number"
          ? val + willAdd
          : willAdd;
    ...
    var isScrollView =
      _component.layout &&
      _component instanceof require("sf-core/ui/scrollview");
  ...
  removeClassNames(classNames) {
    ...
    this.classNames = this.classNames.filter(
      cname => !classNamesArr.some(rname => cname === rname)
    );
  ...
  resetClassNames(classNames = []) {
    this.classNames = [];
    this.initialClassNames
      .concat(classNames)
      .forEach(this.addClassName.bind(this));
    this.isDirty = true;
    return this;
  }
  ...
  pushClassNames(classNames) {
    ...
    if (newClassNames.length) {
      this.classNames = this.classNames.concat(newClassNames);
      this.isDirty = true;
    }
