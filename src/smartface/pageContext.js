...
const buildProps = require("./sfCorePropFactory");
const Screen = require("sf-core/device/screen");
const System = require("sf-core/device/system");
const isTablet = require("../core/isTablet");
const makeStylable = require("../styling/Stylable");
const hooks = require("../core/hooks");
const Contants = require("../core/constants");
const {
  fromSFComponent,
  createActorTreeFromSFComponent
} = require("./fromSFComponent");

var orientationState = "ended";

commands.addRuntimeCommandFactory(function pageContextRuntimeCommandFactory(
  type,
  error
) {
  switch (type) {
          ...
          return function reduceDiffStyleHook(oldStyles, newStyles) {
            ...
            return function diffStylingReducer(acc, key) {
              ...
              if (key === "align") {
                ...
                Object.keys(newStyles[key] || {}).reduce(
                  diffReducer,
                  (acc[key] = {})
                );
...
function contextReducer(context, action, target, state) {
  ...
  switch (action.type) {
    ...
    case "addChild":
      ...
      const ctree = createActorTreeFromSFComponent(
        action.component,
        action.name,
        target
      );
